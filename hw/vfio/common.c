                                           ...
                                           MemoryRegionSection *section)
{
    ...
    ret = vfio_container_dma_unmap(bcontainer, iova, size , NULL, -1);
    if (ret) {
                                     ...
                                     MemoryRegionSection *section)
{
    ...
    if (try_unmap) {
        ...
        if (int128_eq(llsize, int128_2_64())) {
            ...
            ret = vfio_container_dma_unmap(bcontainer, iova,
                                   int128_get64(llsize), NULL, memfd);
            if (ret) {
                error_report("vfio_container_dma_unmap(%p, 0x%"HWADDR_PRIx", "
                             "0x%"HWADDR_PRIx") = %d (%s)",
                           ...
                           void *rsp, size_t rsplen, int *fw_err)
{
    ...
    if (ops->tsm_guest_request) {
        rc = ops->tsm_guest_request(vbasedev, req, reqlen, rsp, rsplen, fw_err);
    }

    trace_vfio_tsm_guest_request(vbasedev->name, vbasedev->fd, rc);
    return rc;
}

int vfio_tsm_remap(VFIODevice *vbasedev, uint64_t addr, Error **errp)
{
    int rc = -ENOTSUP;
    const VFIOIOMMUClass *ops = VFIO_IOMMU_GET_CLASS(vbasedev->bcontainer);

    if (ops->tsm_remap) {
        rc = ops->tsm_remap(vbasedev, addr, errp);
    }

    trace_vfio_tsm_remap(vbasedev->name, addr, rc);
    return rc;
}
