...
static int amd_viommu_update_gcr3(AMDVIState *s, uint64_t gcr3, uint64_t gcr3_va,
                                  uint16_t flags, uint16_t dev_id, uint16_t dom_id,
                                  uint16_t guest_paging_mode)
{
    ...
    hwpt.gdom_id = dom_id;
    hwpt.flags.glx = (flags >> 4) & 0x3;
    hwpt.flags.giov = (flags >> 3) & 0x1;
    hwpt.flags.guest_paging_mode = guest_paging_mode;
...
fprintf(stderr, "DEBUG: %s: ALLOC , gdevid=%#x, glx=%#x\n", __func__,
	dev_id, hwpt.flags.glx);
...
fprintf(stderr, "DEBUG: %s: ATTACH, gdevid=%#x, glx=%#x\n", __func__,
	dev_id, hwpt.flags.glx);

    ret = iommufd_device_attach_hwpt(dev->idev, hwpt_id);
...
//	amd_viommu_update_domain_id(s, devid, domid, true);
    ...
    dev = g_hash_table_lookup(s->amd_iommufd_dev, &key);
    ret = amd_viommu_get_v1_hwpt(dev, s);
    if (ret)
        return;

//SURAVEE
//    ret = iommufd_device_attach_hwpt(dev->idev, dev->v1_hwpt.hwpt_id);
//    if (ret)
//        return;

    fprintf(stderr, "DEBUG: %s: setting gCR3\n", __func__);
                             ...
                             unsigned size)
{
    ...
    case AMDVI_MMIO_STATUS:
        /* TODO: Check this */
        amd_viommu_mmio_write(s, AMDVI_MMIO_STATUS, size, val);
        break;
