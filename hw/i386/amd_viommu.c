                                  ...
                                  uint16_t flags, uint16_t dev_id)
{
    ...
    ret = iommufd_backend_alloc_hwpt(s->iommufd->fd,
                                     ...
                                     sizeof(struct iommu_hwpt_amd_v2),
                                     &hwpt,
                                     &hwpt_id);
out:
    return ret;
...
static int amd_viommu_get_v1_hwpt(AMDIOMMUFDDevice *dev, AMDVIState *s)
{
    ...
    if (!ret) {
        ...
        dev->v1_hwpt.iommufd = dev->idev->iommufd;
        dev->v1_hwpt.parent_id = dev->idev->ioas_id;
    }
    return ret;
}
                             ...
                             unsigned size)
{
    ...
    if (offset % 0x20 != 0x8) {
	error_printf("amd_viommu: Invalid device table offset (%#lx)\n", offset);
        return;
    }
...
#define SUPPORTED_EFR	(FEATURE_GT | FEATURE_GIO | FEATURE_GATS_5LEVEL | \
