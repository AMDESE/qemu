...
insert_viommu_ivhd(PCIBus *bus, PCIDevice *dev, void *opaque)
{
    ...
    /* Only include devices, which are assoicated to the vIOMMU instance */
    if (ivhd->iommu_id != dev->parent_iommu_id)
        return;
...
static void
build_amd_iommu(GArray *table_data, AMDVIState *s, uint64_t efr, uint64_t efr2)
{
